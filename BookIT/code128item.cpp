...
void Code128Item::setPosition(int _x, int _y){
    m_xPos = _x;
    m_yPos = _y;
}
...
void Code128Item::setRotate(bool _rotate){
    m_Rotate = _rotate;
}
...
void Code128Item::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    ...
    for (int i=0;i<m_Code.length();i++)
    {
        ...
        {
            if(m_Rotate){
                QRectF bar(m_yPos, left, m_Height - fontHeight, width);
                painter->fillRect(bar, Qt::SolidPattern);
            } else {
                QRectF bar(left, m_yPos, width, m_Height - fontHeight);
                painter->fillRect(bar, Qt::SolidPattern);
